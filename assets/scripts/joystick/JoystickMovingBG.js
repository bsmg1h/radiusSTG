var Common = require('JoystickMovingCommon');
...
cc.Class({
    ...
    {
        // joy下的Game组件
        this._joyCom = this.node.parent.getComponent('JoystickMoving');
        // game组件下的player节点
    ...
    {
        if (this._speed != 0){
            this._playerNode.getComponent("player").xSpeed += Math.cos(this._angle * (Math.PI/180)) * this._playerNode.getComponent("player").accel * dt;
            this._playerNode.getComponent("player").ySpeed += Math.sin(this._angle * (Math.PI/180)) * this._playerNode.getComponent("player").accel * dt;
        }
    },
